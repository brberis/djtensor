...
def compute_completeness_mm2_for_dataset(
    ...
    def _calibrate_one(img):
        ...
        # The inspector's green outline and the tooth-only cut-outs, from the
        # same code every other outline change uses. This task used to keep
        # its own copy, which never wrote the cut-outs and so left a
        # re-calibrated image showing its old outline.
        try:
